...
•  < 퀵 정렬 >
  ...
  • Pivot값을 기준으로 데이터 묶음을 나누느 작업을 분할(Divide)이라고 함

• 가장 기본적인 퀵 정렬은 첫번째 데이터를 기준 데이터(pivot)으로 설정함
  • ----> 큰 값 찾기 & 작은 값 찾기 <-------  : 그 후 서로 위치를 바꾸기
  • 바꾼 원소 다음부터 화살표 시작점
  • 만약 -> 와 <- 가 엇갈린다면 : pivot과 '작은' 값의 위치를 서로 바꾸기 (만약 pivot == 작은 값 -> 그냥 냅두기)
...
• 최악의 경우 o(N^2)의 시간 복잡도를 가질 수 있음 (N x N) : 한쪽 방향으로 편향된 분할이 발생가능함
...
def quick_sort(array, start, end) : 
    ...
    # 이중 while문 : 1 사이클당 계속 바꾸다가 엇갈리는 순간 stop하고 분할
